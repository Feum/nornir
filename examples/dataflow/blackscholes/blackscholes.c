...
class OptionsStreamRate: public dataflow::InputStreamRate{
private:
    int currentIteration;
    int currentOption;
public:
    inline OptionsStreamRate(std::string fileName):InputStreamRate(fileName),
        currentIteration(0), currentOption(0){
        ;
    }
...
protected:
    std::vector<void*> loadObjects(){
        ...
        while(currentIteration < NUM_RUNS){
            ...
            int availableOnIteration = numOptions - outTask->start;
            while(toAssign > 0 && currentIteration < NUM_RUNS){
                if(toAssign < availableOnIteration){
                    ...
                    availableOnIteration = numOptions;
                    ++currentIteration;
                }
            }

            currentOption = (currentOption + outTask->numElems) % numOptions;
            objects.push_back(outTask);
        }
...
fftask_t* compute(fftask_t* t){
    for(uint i = 0; i < (uint) t->numElems; i++){
         ...
         * Black & Scholes's equation.
         */
        price = BlkSchlsEqEuroNoDiv( sptprice[id], strike[id],
                                     rate[id], volatility[id], otime[id],
                                     otype[id], 0);
        prices[id] = price;
        ...
        if( fabs(priceDelta) >= 1e-4 ){
            printf("Error on %d. Computed=%.5f, Ref=%.5f, Delta=%.5f\n",
                    id, price, data[id].DGrefval, priceDelta);
            numError ++;
        }
#endif
    }
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
   ...
   __parsec_bench_begin(__parsec_blackscholes);
    ...
    if(argc != 5){
                printf("Usage:\n\t%s <nthreads> <inputFile> <outputFile> <rateFile>\n", argv[0]);
                exit(1);
    }
    nThreads = atoi(argv[1]);
    char *inputFile = argv[2];
    char *outputFile = argv[3];
    char *rateFile = argv[4];
